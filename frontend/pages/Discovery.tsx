...
import { fetchAgents, searchAgents, AgentProfile } from '../lib/api';
...
const Discovery = () => {
   ...
   const loadAgents = useCallback(async () => {
      ...
      setError(null);
      try {
         let results: AgentProfile[];
         const query = searchParams.get('q');

         if (query) {
            results = await searchAgents(query);
         } else {
            results = await fetchAgents({
            ...
            <div className="p-6 md:px-10 pb-0">
               ...
               <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-surface-border">
                  <p className="text-white text-sm font-medium">
                     <span className="font-bold">{agents.length}</span> Agents found
                  </p>
               </div>
            </div>
